...
func TestSqlxSpanName(t *testing.T) {
	type args struct {
		method string
		query  string
	}
	...
	tests := []struct {
		...
		{
			name:     "given Get with SELECT query, then returns method with operation",
			args:     args{method: "sqlx.Get", query: "SELECT * FROM users"},
			wantName: "sqlx.Get: SELECT",
		},
		{
			name:     "given Select with SELECT query, then returns method with operation",
			args:     args{method: "sqlx.Select", query: "SELECT * FROM users"},
			wantName: "sqlx.Select: SELECT",
		},
		{
			name:     "given NamedExec with INSERT query, then returns method with operation",
			args:     args{method: "sqlx.NamedExec", query: "INSERT INTO users VALUES (:id)"},
			wantName: "sqlx.NamedExec: INSERT",
		},
		{
			name:     "given empty query, then returns method only",
			args:     args{method: "sqlx.Method", query: ""},
			wantName: "sqlx.Method",
		},
		{
			name:     "given whitespace query, then returns method only",
			args:     args{method: "sqlx.Method", query: "   "},
			wantName: "sqlx.Method",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := sqlxSpanName(tt.args.method, tt.args.query)
			assert.Equal(t, tt.wantName, got)
		})
	}
}
